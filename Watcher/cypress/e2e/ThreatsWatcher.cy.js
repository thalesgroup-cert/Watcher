describe('Threats Watcher - E2E Test Suite', () => {
  before(() => {
    ...
    cy.intercept('GET', '/api/threats_watcher/trendyword/', {
      ...
      body: [
        {
          id: 1, name: "malware", occurrences: 45, score: 85.7,
          posturls: [
        ...
        {
          id: 2, name: "ransomware", occurrences: 32, score: 92.3,
          posturls: [
        ...
        {
          id: 3, name: "phishing", occurrences: 28, score: 78.1,
          posturls: [
    ...
    cy.intercept('GET', '/api/threats_watcher/bannedword/', {
      statusCode: 200,
      body: [
        { id: 1, name: "spam", created_at: "2025-06-19T10:00:00Z" },
        { id: 2, name: "advertisement", created_at: "2025-06-18T15:30:00Z" }
      ]
    }).as('getBannedWords');
  ...
  describe('Page Navigation and Access', () => {
    ...
    it('should load data automatically', () => {
      ...
      cy.get('body').then(($body) => {
        ...
        if (bodyText.includes('malware') || bodyText.includes('ransomware') || bodyText.includes('phishing')) {
    ...
    it('should handle word cloud interactions', () => {
      ...
      cy.get('body').then(($body) => {
        const wordElements = $body.find('text, span, div').filter((index, element) => {
          ...
          return text.includes('malware') || text.includes('ransomware') || text.includes('phishing');
        });
    ...
    it('should display articles table when word is selected', () => {
      cy.get('body').then(($body) => {
        const wordElements = $body.find('td, h5').filter((index, element) => {
          const text = Cypress.$(element).text().toLowerCase();
          return text.includes('malware') || text.includes('ransomware');
        });
  ...
  describe('Performance and Integration Tests', () => {
    ...
    it('should verify Threats Watcher specific components', () => {
      cy.get('body').then(($body) => {
        const bodyText = $body.text();
        if (bodyText.includes('Trendy Words') || bodyText.includes('malware') || bodyText.includes('Article') || bodyText.includes('Clear visited')) {
